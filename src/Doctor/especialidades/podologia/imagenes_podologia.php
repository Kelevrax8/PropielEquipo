<?php
// Imágenes médicas específicas para Podología
session_start();
...
try {
    ...
    // Obtener imágenes médicas filtradas por podología
    $imagenes_podologia = [];
    ...
    try {
        ...
        foreach ($todas_imagenes as $imagen) {
            ...
            // Filtrar imágenes relacionadas con podología usando el campo tipo_imagen
            // Hacer el filtro más flexible para capturar variaciones
            $tipo_lower = strtolower(trim($imagen['tipo_imagen']));
            if ($tipo_lower === 'podologia' || 
                $tipo_lower === 'podología' || 
                strpos($tipo_lower, 'podo') !== false ||
                strpos($tipo_lower, 'pie') !== false) {
                
                $imagenes_podologia[] = $imagen;
        ...
        usort($imagenes_podologia, function($a, $b) {
        ...
        error_log("Error obteniendo imágenes de podología: " . $e->getMessage());
    }
    
    // Filtrar por paciente seleccionado si se especifica
    $paciente_seleccionado = $_GET['paciente'] ?? '';
    $imagenes_filtradas = $imagenes_podologia;
    
    if (!empty($paciente_seleccionado) && $paciente_seleccionado !== 'todos') {
        $imagenes_filtradas = array_filter($imagenes_podologia, function($imagen) use ($paciente_seleccionado) {
            return $imagen['id_paciente'] == $paciente_seleccionado;
        });
    }
    
} catch (Exception $e) {
    error_log("Error en imágenes podología: " . $e->getMessage());
    header('location: ../../../Landing/login.html');
    exit();
}
?>
...
<head>
    ...
    <title>Imágenes Podología - PropielEquipo</title>
                ...
                <div class="flex items-center gap-4">
                    <form method="GET" class="flex items-center gap-3">
                        <select name="paciente" onchange="this.form.submit()" 
                                class="bg-white border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
                            <option value="todos" <?php echo ($paciente_seleccionado === '' || $paciente_seleccionado === 'todos') ? 'selected' : ''; ?>>
                                Todos los pacientes (<?php echo count($imagenes_podologia); ?> imágenes)
                            </option>
            ...
            <div class="bg-white rounded-xl p-6 shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-emerald-600"><?php echo count($imagenes_filtradas); ?></p>
                    </div>
                    <ion-icon name="images-outline" class="text-3xl text-emerald-500"></ion-icon>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="bg-emerald-600 text-white p-6">
                <h2 class="text-xl font-bold flex items-center">
                    ...
                    <?php else: ?>
                        Galería de Imágenes Podológicas
                        <span class="ml-2 text-sm bg-emerald-500 px-2 py-1 rounded">
    ...
    <?php endif; ?>
                                ...
                                <div class="p-4">
                                    <div class="flex justify-between items-start mb-2">
                                        ...
                                        <span class="bg-emerald-100 text-emerald-800 px-2 py-1 text-xs font-semibold rounded">
                ...
                <div class="p-12 text-center">
                    <div class="text-gray-500">
                        ...
                        <?php if (!empty($paciente_seleccionado) && $paciente_seleccionado !== 'todos'): ?>
                            ...
                            <p class="text-sm">Este paciente no tiene imágenes de podología registradas</p>
                            <div class="mt-4">
                                <a href="?paciente=todos" 
                        ...
                        <?php else: ?>
                            <p class="text-lg">No hay imágenes de podología registradas</p>
