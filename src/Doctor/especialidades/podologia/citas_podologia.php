<?php
// Citas específicas para Podología
session_start();
...
try {
    ...
    // Obtener todas las citas de podología asignadas a este doctor
    $citas_podologia = [];
    ...
    try {
        ...
        foreach ($todas_citas as $cita) {
            if (strtolower($cita['servicio']) === 'podología') {
                $citas_podologia[] = $cita;
            }
        }
        
        // Ordenar por fecha más reciente
        usort($citas_podologia, function($a, $b) {
            // Ordenar por fecha de creación (más reciente primero)
            return strtotime($b['fecha_creacion']) - strtotime($a['fecha_creacion']);
        });
        
    } catch (Exception $e) {
        error_log("Error obteniendo citas de podología: " . $e->getMessage());
    }
    
} catch (Exception $e) {
    error_log("Error en citas podología: " . $e->getMessage());
    header('location: ../../../Landing/login.html');
    exit();
}
?>
...
<head>
    ...
    <title>Citas Podología - PropielEquipo</title>
...
<body class="bg-gradient-to-r from-emerald-50 to-emerald-100 flex flex-col min-h-screen">
    ...
    <!-- Header de Citas de Podología -->
    <div class="container mx-auto px-4 py-8">
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-2xl p-6 text-white mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold mb-2">Citas de Podología</h1>
                    <p class="text-emerald-100">Dr. <?php echo htmlspecialchars($nombre . " " . $apellido); ?></p>
                    <p class="text-emerald-200 text-sm">Gestión de citas podológicas</p>
                </div>
            ...
            <div class="bg-white rounded-xl p-6 shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-emerald-600"><?php echo count($citas_podologia); ?></p>
                    </div>
                    <ion-icon name="calendar-outline" class="text-3xl text-emerald-500"></ion-icon>
                </div>
            </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">
                            <?php 
                            $citas_hoy_pendientes = array_filter($citas_podologia, function($cita) use ($hoy, $datetime_actual) {
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-yellow-600">
                            <?php 
                            $citas_futuras = array_filter($citas_podologia, function($cita) use ($datetime_actual) {
                                $datetime_cita = DateTime::createFromFormat('Y-m-d H:i', $cita['fecha'] . ' ' . $cita['horario']);
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-emerald-600">
                            <?php 
                            $citas_completadas = array_filter($citas_podologia, function($cita) use ($datetime_actual) {
                                $datetime_cita = DateTime::createFromFormat('Y-m-d H:i', $cita['fecha'] . ' ' . $cita['horario']);
                                return $datetime_cita < $datetime_actual;
                            });
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="bg-emerald-600 text-white p-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <!-- Week Navigator -->
                    <div class="flex items-center gap-2 bg-emerald-700 rounded-lg px-4 py-2">
                        <button onclick="changeWeek(-1)" class="text-white hover:text-emerald-200 transition">
                        ...
                        <button onclick="goToToday()" class="ml-2 text-white hover:text-emerald-200 transition text-sm underline">
                            Hoy
                        </button>
                    </div>
                </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-b flex gap-2">
                <button onclick="setViewMode('week')" id="week-view-btn" class="px-4 py-2 rounded-lg text-sm font-medium transition bg-emerald-600 text-white">
            ...
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-200" id="pagination-info">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-600">
                        Mostrando <span id="showing-start">0</span> a <span id="showing-end">0</span> de <span id="total-appointments"><?php echo count($citas_podologia); ?></span> citas
                    </div>
        ...
        <div class="mt-8 flex flex-wrap gap-4">
            <a href="dashboard_podologia.php" 
            ...
            <a href="pacientes_podologia.php" 
...
<div id="observations-modal" class="fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 hidden p-4">
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[95vh] flex flex-col">
        ...
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white p-4 sm:p-6 flex-shrink-0 rounded-t-lg">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    <h2 class="text-lg sm:text-xl font-bold truncate">Observaciones Dermatológicas</h2>
                    <p id="patient-info" class="text-emerald-100 text-xs sm:text-sm truncate">Cargando información del paciente...</p>
                </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 sm:p-6">
            ...
            <div class="bg-emerald-50 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-xs sm:text-sm">
                    ...
                    <div class="break-words">
                        ...
                        <span id="modal-appointment-service" class="ml-2">Podología</span>
                    </div>
                    <div class="break-words">
            ...
            <div class="mb-4 sm:mb-6">
                <label for="observations-textarea" class="block text-sm font-semibold text-gray-700 mb-2">
                    <ion-icon name="walk" class="mr-2 text-emerald-600"></ion-icon>
                ...
                <textarea 
                    ...
                    class="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none text-sm"
                ></textarea>
        ...
        <div class="border-t bg-gray-50 px-4 sm:px-6 py-3 sm:py-4 flex-shrink-0 rounded-b-lg">
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-end">
                ...
                <button 
                    ...
                    class="px-4 sm:px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition duration-200 text-sm sm:text-base">
...
<script>
    ...
    const allAppointments = <?php echo json_encode($citas_podologia); ?>;
    ...
    function setViewMode(mode) {
        ...
        if (mode === 'week') {
            weekBtn.className = 'px-4 py-2 rounded-lg text-sm font-medium transition bg-emerald-600 text-white';
            ...
            allBtn.className = 'px-4 py-2 rounded-lg text-sm font-medium transition bg-emerald-600 text-white';
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                            ...
                            <div class="flex items-start gap-4 flex-1">
                                ...
                                <div class="flex-shrink-0">
                                    <div class="bg-emerald-100 text-blue-800 font-bold text-lg px-3 py-2 rounded-lg text-center min-w-[70px]">
                                        ${cita.horario}
                                    </div>
                                </div>
                            ...
                            <div class="flex-shrink-0">
                                <button onclick="openObservationsModal(${cita.id_cita})" 
                                        class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm transition duration-200 flex items-center w-full lg:w-auto justify-center">
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="text-gray-500">
                            <ion-icon name="calendar-outline" class="text-6xl text-gray-300 mb-4"></ion-icon>
                            <p class="text-lg">No hay citas de podología registradas</p>
            ...
            return `
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button onclick="openObservationsModal(${cita.id_cita})" 
                                class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1 rounded text-sm transition duration-200 flex items-center">
            ...
            if (i === 1 || i === totalPages || (i >= currentPage - 1 && i <= currentPage + 1)) {
                buttons.push(`
                    <button onclick="changePage(${i})" 
                            class="px-3 py-1 rounded text-sm ${i === currentPage ? 'bg-emerald-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100 border'}">
                        ${i}
                    </button>
                `);
