<?php
// Imágenes médicas específicas para Dermatología
session_start();
...
try {
    ...
    // Obtener imágenes médicas filtradas por dermatología
    $imagenes_dermatologia = [];
    ...
    try {
        ...
        foreach ($todas_imagenes as $imagen) {
            ...
            // Filtrar imágenes relacionadas con dermatología usando el campo tipo_imagen
            // Hacer el filtro más flexible para capturar variaciones
            $tipo_lower = strtolower(trim($imagen['tipo_imagen']));
            if ($tipo_lower === 'dermatologia' || 
                $tipo_lower === 'dermatología' || 
                strpos($tipo_lower, 'derma') !== false ||
                empty($imagen['tipo_imagen'])) { // Incluir imágenes sin tipo específico
                
                $imagenes_dermatologia[] = $imagen;
        ...
        usort($imagenes_dermatologia, function($a, $b) {
        ...
        error_log("Error obteniendo imágenes de dermatología: " . $e->getMessage());
    }
    
    // Filtrar por paciente seleccionado si se especifica
    $paciente_seleccionado = $_GET['paciente'] ?? '';
    $imagenes_filtradas = $imagenes_dermatologia;
    
    if (!empty($paciente_seleccionado) && $paciente_seleccionado !== 'todos') {
        $imagenes_filtradas = array_filter($imagenes_dermatologia, function($imagen) use ($paciente_seleccionado) {
            return $imagen['id_paciente'] == $paciente_seleccionado;
        });
    }
    
} catch (Exception $e) {
    error_log("Error en imágenes dermatología: " . $e->getMessage());
    header('location: ../../../Landing/login.html');
    exit();
}
?>
...
<head>
    ...
    <title>Imágenes Dermatología - PropielEquipo</title>
                ...
                <div class="flex items-center gap-4">
                    <form method="GET" class="flex items-center gap-3">
                        <select name="paciente" onchange="this.form.submit()" 
                                class="bg-white border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <option value="todos" <?php echo ($paciente_seleccionado === '' || $paciente_seleccionado === 'todos') ? 'selected' : ''; ?>>
                                Todos los pacientes (<?php echo count($imagenes_dermatologia); ?> imágenes)
                            </option>
        ...
        <div class="grid md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white rounded-xl p-6 shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-blue-600"><?php echo count($imagenes_filtradas); ?></p>
                    </div>
                    <ion-icon name="images-outline" class="text-3xl text-blue-500"></ion-icon>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 shadow-lg">
                <div class="flex items-center justify-between">
                    ...
                    <ion-icon name="today" class="text-3xl text-purple-500"></ion-icon>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 shadow-lg">
                <div class="flex items-center justify-between">
                    ...
                    <ion-icon name="people" class="text-3xl text-orange-500"></ion-icon>
                </div>
            </div>
        </div>
            ...
            <div class="bg-blue-600 text-white p-6">
                <h2 class="text-xl font-bold flex items-center">
                    ...
                    <?php if (!empty($paciente_seleccionado) && $paciente_seleccionado !== 'todos'): ?>
                        ...
                        <span class="ml-2 text-sm bg-blue-500 px-2 py-1 rounded">
                            <?php echo count($imagenes_filtradas); ?> imágenes
                        </span>
                    <?php else: ?>
                        Galería de Imágenes Dermatológicas
                        <span class="ml-2 text-sm bg-blue-500 px-2 py-1 rounded">
...
<div class="relative h-48 bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
    ...
    <?php endif; ?>
                                ...
                                <div class="p-4">
                                    <div class="flex justify-between items-start mb-2">
                                        ...
                                        <span class="bg-blue-100 text-blue-800 px-2 py-1 text-xs font-semibold rounded">
                ...
                <div class="p-12 text-center">
                    <div class="text-gray-500">
                        ...
                        <?php if (!empty($paciente_seleccionado) && $paciente_seleccionado !== 'todos'): ?>
                            ...
                            <p class="text-sm">Este paciente no tiene imágenes de dermatología registradas</p>
                            <div class="mt-4">
                                <a href="?paciente=todos" 
                        ...
                        <?php else: ?>
                            <p class="text-lg">No hay imágenes de dermatología registradas</p>
        ...
        <div class="max-w-4xl max-h-full p-4 relative">
            <div class="bg-white rounded-xl overflow-hidden shadow-2xl">
                <div class="flex items-center justify-between p-4 bg-blue-600 text-white">
                    <h3 class="text-lg font-bold" id="modalTitulo">Imagen Médica - Dermatología</h3>
    ...
    <script>
        ...
        function verImagenDetalle(nombreArchivo, idImagen) {
            ...
            modalTitulo.textContent = 'Imagen Médica #' + idImagen + ' - Dermatología';
