<?php
/**
 * Endpoint para obtener el historial médico de un paciente de Podología
 */
...
try {
    ...
    // Verificar que el médico tenga especialidad en podología
    ...
    if (isset($_SESSION['especialidades']) && is_array($_SESSION['especialidades'])) {
        foreach ($_SESSION['especialidades'] as $esp) {
            if (isset($esp['id_especialidad']) && $esp['id_especialidad'] == 2) { // ID 2 = Podología
                $tiene_podologia = true;
                $especialidad_medico = 'Podología';
                break;
            }
        }
    }
    
    // Si no tiene especialidad en podología, no debería poder acceder
    if (!$tiene_podologia) {
        sendResponse(false, null, 'No tiene permisos para acceder a historiales de podología');
    }
    ...
    foreach ($todas_citas as $cita) {
        $citas_podologia[] = [
            ...
            'servicio' => $cita['servicio'] ?? 'Podología',
    ...
    sendResponse(true, [
        ...
        'appointments' => $citas_podologia,
        'total_appointments' => count($citas_podologia)
    ], count($citas_podologia) > 0 ? 
        'Se encontraron ' . count($citas_podologia) . ' citas de podología' : 
        'No se encontraron citas de podología para este paciente');
    
} catch (Exception $e) {
    error_log("Error en get_patient_history_podologia.php: " . $e->getMessage());
